...
public class CartesianDataManager {
	...
	public static void addFunction(PlotPage page)
		...
		if (dialog.getPressed() == CartesianFunctionDialog.ADD_PRESSED) {
			// get new cartesian function
			CartesianFunction function = dialog.getFunction();
			// add function to page list and gui list
	...
	public static void editDataSet(PlotPage page, int selectedIndex)
		...
		CartesianDataSet currentDataSet = (CartesianDataSet) OpenPlotTool
				.getMainFrame().getDataSetList().getSelectedValue();
		if (currentDataSet != null) {
			// create and display a EditCartesianDataDialog
	...
	public static void editFunction(PlotPage page, int selectedIndex)
		...
		CartesianFunction currentFunction = (CartesianFunction) OpenPlotTool
				.getMainFrame().getFunctionList().getSelectedValue();
		if (currentFunction != null) {
			// create and display a EditCartesianFunctionDialog
			CartesianFunctionDialog dialog = new CartesianFunctionDialog(
					currentFunction);
			dialog.setVisible(true);
			if (dialog.getPressed() == CartesianFunctionDialog.ADD_PRESSED) {
				// get new cartesian function
				CartesianFunction newFunction = dialog.getFunction();
				// update function in lists
	...
	public static void editPoint(PlotPage page, int selectedIndex)
		...
		// get selected point
		CartesianPoint currentPoint = (CartesianPoint) OpenPlotTool
				.getMainFrame().getPointList().getSelectedValue();
		if (currentPoint != null) {
			// create and display a EditCartesianPointDialog
			CartesianPointDialog dialog = new CartesianPointDialog(currentPoint);
			dialog.setVisible(true);
			if (dialog.getPressed() == CartesianFunctionDialog.ADD_PRESSED) {
				// get new cartesian point
				CartesianPoint newPoint = dialog.getPoint();
				// update point in lists
				...
				OpenPlotTool.getMainFrame().getPointListModel().removeElement(
						currentPoint);
				OpenPlotTool.getMainFrame().getPointListModel().add(
						selectedIndex, newPoint);
				// repaint page
				page.repaint();
	...
	public static void removeDataSet(PlotPage page) {
		// get selected data set
		PlotDataSet dataSet = (PlotDataSet) OpenPlotTool.getMainFrame()
	...
	public static void removeFunction(PlotPage page) {
		// get selected function
		PlotFunction function = (PlotFunction) OpenPlotTool.getMainFrame()
				.getFunctionList().getSelectedValue();
		if (function != null) {
	...
	public static void removePoint(PlotPage page) {
		// get selected point
		PlotPoint point = (PlotPoint) OpenPlotTool.getMainFrame()
				.getPointList().getSelectedValue();
		if (point != null) {
			// remove function from lists
